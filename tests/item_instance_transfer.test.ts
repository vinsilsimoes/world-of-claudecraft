...
// (server/entity_presentation_wire.ts), the enchant_apply_view.test.ts
// precedent: widen both or neither.
...
describe('publicInstanceView: the display trim', () => {
  ...
  it('is the single eqi wire projection: widen one allowlist, not two', () => {
    const wire = readFileSync(
      new URL('../server/entity_presentation_wire.ts', import.meta.url),
      'utf8',
    );
    expect(wire).toContain(
      "import { publicInstanceView } from '../src/sim/item_instance_transfer'",
    );
    expect(wire).toContain('const publicInstance = publicInstanceView(instance);');
