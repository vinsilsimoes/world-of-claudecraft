...
describe('createWsAuth: authenticateWebSocket reject paths', () => {
  ...
  it('2c. rejects an auth-world-13 client on the auth-world-14 server before all admission work', async () => {
    const fixture = setup();
    const { ws, deps, req } = fixture;

    await createWsAuth(deps).authenticateWebSocket(
      asWs(ws),
      JSON.stringify({ t: 'auth-world-13', token: 'tok', character: 7 }),
      req,
    );
  ...
  it.each(['auth-world', 'auth-world-15', 'auth-world-next', 'auth-world-01', 'auth-world-1.0'])(
