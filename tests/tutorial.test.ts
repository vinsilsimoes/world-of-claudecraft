...
describe('isFreshCharacter', () => {
  ...
  const world = (
    ...
    ({
      ...
      cfg: { seed: 1, playerClass: 'warrior', gameProfile: 'woc-classic' },
  ...
  it('never engages the Eastbrook tutorial in the MIR4 profile', () => {
    expect(
      isFreshCharacter(
        world({ cfg: { seed: 1, playerClass: 'warrior', gameProfile: 'mir4-gameplay-port' } }),
      ),
    ).toBe(false);
  });
});
