...
describe('the complete MIR4 Sorcerer kit', () => {
  ...
  it('explodes Flame Orb into nearby enemies and burns every enemy hit', () => {
    ...
    expect(castMir4Skill(sim.ctx, sim.playerId, 2101, primary.id)).toEqual({ ok: true });
  ...
  it('pulls enemies into Dark Vortex and knocks them away with Flame Strike', () => {
    ...
    expect(castMir4Skill(flameStrike.ctx, flameStrike.playerId, 2201)).toEqual({ ok: true });
  ...
  it('buffs spell damage with Phoenix Embrace and protects with Frozen Block', () => {
    ...
    expect(castMir4Skill(phoenix.ctx, phoenix.playerId, 2204)).toEqual({ ok: true });
    ...
    expect(castMir4Skill(frozen.ctx, frozen.playerId, 2202)).toEqual({ ok: true });
    expect(frozen.player.mir4Shield).toMatchObject({ remaining: 4, magnitude: 1 });
