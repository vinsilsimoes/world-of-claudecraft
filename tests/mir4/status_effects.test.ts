...
describe("MIR4 recovery and action modifiers", () => {
  it("combines global and potion-specific recovery rates", () => {
    expect(mir4ModifiedPotionAmount(500, "hp", { 94: 1_000, 146: 2_000 })).toBe(
      650,
    );
    expect(mir4ModifiedPotionAmount(120, "mp", { 94: 1_000, 147: 1_500 })).toBe(
      150,
    );
    expect(mir4ModifiedPotionAmount(1_000, "hp", undefined, -1_000)).toBe(900);
  });
  ...
  it("calculates ten-second recovery and attack drain with integer flooring", () => {
    ...
    expect(mir4DrainOnDamage(999, { 80: 500, 81: 250 })).toEqual({
      hp: 49,
      mp: 24,
    });
    expect(mir4DrainOnDamage(1_000, { 80: 9_999, 81: 9_999 })).toEqual({
      hp: 200,
      mp: 150,
    });
    ...
    expect(mir4ModifiedSkillHealing(1_000, { 148: 2_500 }, 1_500)).toBe(1_400);
...
describe("MIR4 progression modifiers", () => {
  it("keeps hunting and reward lanes separate and combines both hunting XP statuses", () => {
    ...
    expect(mir4ModifiedProgressionReward(1_000, "hunting-xp", statuses)).toBe(
      1_300,
    );
    expect(mir4ModifiedProgressionReward(1_000, "reward-xp", statuses)).toBe(
      1_300,
    );
    expect(
      mir4ModifiedProgressionReward(1_000, "hunting-copper", statuses),
    ).toBe(1_050);
    expect(
      mir4ModifiedProgressionReward(1_000, "reward-copper", statuses),
    ).toBe(1_000);
  });
  ...
  it("boosts drop chance and gathering speed without exceeding probability one", () => {
    ...
    expect(mir4ModifiedDropChance(0.25, { 88: 2_000, 89: 300 })).toBeCloseTo(
      0.3075,
      10,
    );
    expect(mir4ModifiedDropChance(0.9, { 88: 5_000 })).toBe(1);
    expect(mir4ModifiedGatherDurationSeconds(5, "mining", { 93: 2_500 })).toBe(
      4,
    );
  });
