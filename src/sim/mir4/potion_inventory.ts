...
export function mir4PotionBarItems(
  ...
  for (const slot of inventory) {
    ...
    if (rule && playerLevel < rule.requiredLevel) continue;
    if ((item.potionHp ?? 0) > 0) {
      healing = strongerPotion(
        healing,
        item,
        (candidate) => mir4PotionRule(candidate.id)?.restoreBps ?? candidate.potionHp ?? 0,
      );
    }
    if ((item.potionMana ?? 0) > 0) {
      mana = strongerPotion(
        mana,
        item,
        (candidate) => mir4PotionRule(candidate.id)?.restoreBps ?? candidate.potionMana ?? 0,
      );
    }
  }
...
export function useMir4AutomaticPotion(ctx: SimContext, pid: number, kind: 'hp' | 'mp'): boolean {
  ...
  const potionIds = mir4PotionBarItems(meta.inventory, (itemId) => ITEMS[itemId], [], player.level);
